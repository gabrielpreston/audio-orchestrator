...
DOCKER_COMPOSE := $(shell if command -v docker-compose >/dev/null 2>&1; then echo "docker-compose"; else echo ""; fi)
...
run: stop ## Start docker-compose stack (Discord bot + STT + orchestrator)
	...
	@if [ "$(HAS_DOCKER_COMPOSE)" = "0" ]; then echo "'docker-compose' was not found; please install Docker Compose."; exit 1; fi

...
# Token configuration
TOKEN_LENGTH = 32
TOKEN_ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"  # noqa: S105  # nosec B105
...
def update_token_in_lines(
    lines: list[str], token_name: str, new_value: str
) -> list[str]:
...
def validate_token_consistency() -> bool:
    ...
    for file_path_str, _ in ENV_FILES.items():
        ...
        for line in lines:
            ...
            for token_name in AUTH_TOKENS:
                if stripped_line.startswith(f"{token_name}="):

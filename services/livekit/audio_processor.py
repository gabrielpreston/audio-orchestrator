...
from services.common.audio_contracts import CANONICAL_FRAME_MS, CANONICAL_SAMPLE_RATE, AudioFrame
...
class LiveKitAudioProcessor:
    ...
    async def encode_pcm_to_opus(
        self, pcm_data: bytes, sample_rate: int = CANONICAL_SAMPLE_RATE
    ) -> bytes:
    ...
    def resample_audio(
        self, pcm_data: bytes, from_rate: int, to_rate: int = CANONICAL_SAMPLE_RATE
    ) -> bytes:
        ...
        return self.audio_processor.resample_audio(
            pcm_data, from_rate=from_rate, to_rate=to_rate, channels=1, sample_width=2
        )

...
class LiveKitSTTAdapter(STTAdapter):
    ...
    async def start_stream(self, correlation_id: str) -> str:
        ...
        self.logger.info("stt.stream_started", stream_id=stream_id, correlation_id=correlation_id)
    ...
    async def flush_stream(self, stream_id: str) -> Optional[AudioSegment]:
        ...
        self.logger.info(
            "stt.stream_flushed", stream_id=stream_id, correlation_id=stream_data["correlation_id"]
        )
    ...
    async def _process_frame_batch(
        self, stream_id: str, is_final: bool = False
    ) -> Optional[AudioSegment]:
    ...
    async def _frames_to_wav(self, frames: List[AudioFrame]) -> bytes:
        ...
        return processor.pcm_to_wav(
            pcm_data, sample_rate=frames[0].sample_rate, channels=1, sample_width=2
        )

...
class LiveKitTTSAdapter(TTSAdapter):
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.post(
                    ...
                    json={"text": text, "voice": voice, "stream": False},
                )
            ...
            self.logger.error(
                "tts.synthesis_error", correlation_id=correlation_id, error=str(e), text=text[:100]
            )
            raise RuntimeError(f"TTS synthesis failed: {e}")
        except Exception as e:
            self.logger.error(
                "tts.unexpected_error", correlation_id=correlation_id, error=str(e), text=text[:100]
            )
            raise
    ...
    async def start_stream(self, correlation_id: str) -> str:
        ...
        self.logger.info("tts.stream_started", stream_id=stream_id, correlation_id=correlation_id)
    ...
    async def add_text_chunk(self, stream_id: str, text: str) -> None:
        ...
        try:
            audio_data = await self.synthesize_text(
                text, correlation_id=stream_data["correlation_id"]
            )
        ...
        if stream_id in self._active_streams:
            ...
            if stream_data["text_chunks"]:
                ...
                if remaining_text.strip():
                    try:
                        audio_data = await self.synthesize_text(
                            remaining_text, correlation_id=stream_data["correlation_id"]
                        )
                        stream_data["audio_chunks"].append(audio_data)
                    except Exception as e:
                        self.logger.error(

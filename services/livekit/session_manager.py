...
class MobileSession:
    ...
    def can_barge_in(self) -> bool:
        ...
        return (
            ...
            and not self.response_paused
            and (time.time() - self.pause_start_time) < self.max_pause_duration
        )
...
class SessionManager:
    ...
    async def create_session(
        self, room: rtc.Room, participant: rtc.RemoteParticipant, correlation_id: str
    ) -> MobileSession:
    ...
    async def remove_session(self, correlation_id: str) -> bool:
        ...
        self.logger.info(
            "session.removed", correlation_id=correlation_id, total_sessions=len(self._sessions)
        )
    ...
    async def handle_control_message(
        self, session: MobileSession, message: ControlMessage
    ...
    async def _handle_wake_detected(
        ...
        self.logger.info(
            "session.wake_detected", correlation_id=session.correlation_id, confidence=confidence
        )
        ...
        return EndpointingMessage(session.correlation_id, session.endpointing_state.value)
    ...
    async def _handle_vad_start_speech(
        ...
        if session.is_responding and session.can_barge_in():
            ...
            self.logger.info("session.barge_in_triggered", correlation_id=session.correlation_id)

            return PlaybackControlMessage(
                session.correlation_id, PlaybackAction.PAUSE.value, "user_speaking"
            )
    ...
    async def _handle_vad_end_speech(
        ...
        self.logger.info(
            "session.speech_ended", correlation_id=session.correlation_id, duration_ms=duration_ms
        )
    ...
    async def _handle_barge_in_request(
        ...
        if session.can_barge_in():
            ...
            self.logger.info(
                "session.barge_in_requested", correlation_id=session.correlation_id, reason=reason
            )

            return PlaybackControlMessage(
                session.correlation_id, PlaybackAction.PAUSE.value, reason
            )
    ...
    async def _cleanup_session(self, session: MobileSession):
        ...
        try:
            ...
            self.logger.error(
                "session.cleanup_error", correlation_id=session.correlation_id, error=str(e)
            )
        ...
        while True:
            try:
                ...
                for session in self._sessions.values():
                    if (
                        session.is_expired() or (current_time - session.last_activity) > 300
                    ):  # 5 minutes

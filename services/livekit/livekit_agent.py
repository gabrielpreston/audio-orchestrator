...
class MobileVoiceAgent:
    ...
    async def _handle_data_received(self, data: rtc.DataPacket):
        ...
        try:
            ...
            from ..common.audio_contracts import MessageType, ControlMessage
    ...
    async def _handle_track_subscribed(self, track: rtc.Track, publication: rtc.TrackPublication, participant: rtc.RemoteParticipant):
    ...
    async def _handle_track_unsubscribed(self, track: rtc.Track, publication: rtc.TrackPublication, participant: rtc.RemoteParticipant):
    ...
    async def _process_transcript(self, session: MobileSession, segment: AudioSegment):
        ...
        try:
            ...
            if not segment.is_final:
                ...
                if self.data_channel:
                    await self.data_channel.send(
                        json.dumps(partial_msg.to_dict()).encode('utf-8')
                    )
            ...
            if self.data_channel:
                await self.data_channel.send(
                    json.dumps(final_msg.to_dict()).encode('utf-8')
                )
    ...
    async def _call_orchestrator(self, session: MobileSession, segment: AudioSegment):
        ...
        try:
            ...
            if self.data_channel:
                await self.data_channel.send(
                    json.dumps(error_msg.to_dict()).encode('utf-8')
                )

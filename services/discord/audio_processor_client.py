...
class AudioProcessorClient:
    ...
    async def process_frame(self, frame: PCMFrame) -> PCMFrame | None:
        ...
        try:
            ...
            response = await self._make_request_with_retry(
                "POST", f"{self.base_url}/process/frame", json=request_data
            )
    ...
    async def process_segment(self, segment: AudioSegment) -> AudioSegment | None:
        ...
        try:
            ...
            # Make request to audio processor
            response = await self._make_request_with_retry(
                "POST", f"{self.base_url}/process/segment", json=request_data
            )
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    self._logger.warning(
                        "audio_processor_client.retry_attempt",
                        attempt=attempt + 1,
                    ...
                    self._logger.error(
                        "audio_processor_client.max_retries_exceeded",
                        max_retries=self.max_retries,
                        error=str(exc),
                    )

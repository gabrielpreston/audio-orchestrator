...
class ConfigMigrationValidator:
    ...
    def validate_migration(
        ...
        try:
            ...
            for old_section, old_values in old_config.items():
                if old_section in self.migration_map:
                    ...
                    for old_key, new_env_var in migration_rules.items():
                        if old_key in old_values and not self._has_equivalent_value(
                            old_values[old_key], new_env_var, new_config
                        ):
                            logger.warning(
            ...
            logger.error("config.migration_validation_failed", error=str(exc))
            return False

    def _has_equivalent_value(
        self, old_value: Any, new_env_var: str, new_config: dict[str, Any]
    ) -> bool:
    ...
    def _validate_new_config(self, new_config: dict[str, Any]) -> bool:
        ...
        for section in required_sections:
            if section not in new_config:
                logger.warning("config.migration_missing_section", section=section)
                return False
...
def migrate_config_file(old_config_path: Path, new_config_path: Path) -> bool:
    ...
    try:
        ...
        with Path(old_config_path).open() as f:
        ...
        new_config = {
            "logging": {
                ...
                "service_name": old_config.get("logging", {}).get(
                    "service_name", "audio-orchestrator"
                ),
            },
            "http": {
            ...
            "audio": {
                ...
                "enable_enhancement": old_config.get("audio", {}).get(
            ...
            "telemetry": {
                ...
                "metrics_port": old_config.get("telemetry", {}).get(
                    "metrics_port", 9090
                ),
                "jaeger_endpoint": old_config.get("telemetry", {}).get(
                    "jaeger_endpoint", ""
                ),
            },
        }
        ...
        with Path(new_config_path).open("w") as f:

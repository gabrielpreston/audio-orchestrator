...
def example_stt_service_config():
    ...
    config = (
        builder
        .add_config("logging", config.configs["logging"])
...
def example_tts_service_config():
    ...
    config = (
        builder
        .add_config("logging", config.configs["logging"] if "config" in locals() else None)
...
def example_orchestrator_service_config():
    ...
    config = (
        builder
        .add_config("logging", TelemetryConfig)  # Use TelemetryConfig for logging
        .add_config("http", config.configs["http"] if "config" in locals() else None)

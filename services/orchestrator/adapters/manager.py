...
class AdapterManager:
    ...
    def get_input_adapter(self, name: str) -> Optional[AudioInputAdapter]:
    ...
    def get_output_adapter(self, name: str) -> Optional[AudioOutputAdapter]:
    ...
    def unregister_input_adapter(self, name: str) -> bool:
        ...
        self._logger.info(
            ...
            extra={"adapter_name": name, "total_input_adapters": len(self._input_adapters)}
        )
        return True
    ...
    def unregister_output_adapter(self, name: str) -> bool:
        ...
        self._logger.info(
            ...
            extra={"adapter_name": name, "total_output_adapters": len(self._output_adapters)}
        )
        return True
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        for adapter in self._output_adapters.values():
            try:
                adapter_health = await adapter.health_check()
                output_health[adapter.name] = adapter_health
            except Exception as e:
                self._logger.warning(
                    "Error during output adapter health check",
                    extra={"adapter_name": adapter.name, "error": str(e)}
                )
                output_health[adapter.name] = {
                    "status": "unhealthy",
                    "error": str(e)
                }

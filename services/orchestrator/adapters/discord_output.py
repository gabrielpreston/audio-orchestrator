...
class DiscordAudioOutputAdapter(AudioOutputAdapter):
    ...
    def __init__(self, config: AdapterConfig) -> None:
        ...
        self._playback_queue: asyncio.Queue[AudioChunk] = asyncio.Queue()
    ...
    async def start_playback(self) -> None:
        ...
        try:
            ...
            self._logger.error(
                "Failed to start Discord audio playback", extra={"error": str(e)}
            )
            raise
    ...
    async def stop_playback(self) -> None:
        ...
        try:
            ...
            self._logger.error(
                "Error stopping Discord audio playback", extra={"error": str(e)}
            )
    ...
    async def play_audio(self, audio_chunk: AudioChunk) -> None:
        ...
        try:
            ...
            self._logger.error(
                "Error playing audio chunk to Discord", extra={"error": str(e)}
            )
            raise
    ...
    async def play_audio_stream(self, audio_stream: AsyncIterator[AudioChunk]) -> None:
        ...
        try:
            ...
            self._logger.error(
                "Error playing audio stream to Discord", extra={"error": str(e)}
            )
            raise
            ...
            while self._is_playing:
                try:
                    ...
                    audio_chunk = await asyncio.wait_for(
                        self._playback_queue.get(), timeout=1.0
                    )
                    ...
                    self._logger.warning(
                        "Error in Discord audio playback loop", extra={"error": str(e)}
                    )
                    continue
    ...
    async def _play_audio_to_discord(self, audio_chunk: AudioChunk) -> None:
        ...
        try:
            ...
            self._logger.error(
                "Error playing audio to Discord", extra={"error": str(e)}
            )
            raise
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        discord_health = {
            ...
            "playback_task_running": self._playback_task is not None

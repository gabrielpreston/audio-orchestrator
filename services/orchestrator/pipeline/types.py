...
class ProcessingConfig:
    ...
    wake_phrases: list[str] = field(
        default_factory=lambda: ["hey assistant", "computer"]
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.min_segment_duration >= self.max_segment_duration:
            raise ValueError(
                "min_segment_duration must be less than max_segment_duration"
            )
...
class ProcessedSegment:
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            self.clarity_score >= 0.7
            and self.noise_level <= 0.3
            and self.volume_level >= 0.1
        )

...
class AudioProcessor:
    ...
    async def process_audio_chunk(
        ...
        try:
            ...
            processed_data = await self._resample_audio(processed_data, audio_chunk.metadata)
            ...
            processed_segment = ProcessedSegment(
                ...
                duration=len(processed_data) / (self.config.target_sample_rate * self.config.target_channels * 2),  # 16-bit
                ...
                metadata={
                    ...
                    "processing_stages": [
                        "format_conversion",
                        "resampling",
                        "normalization" if self.config.enable_volume_normalization else None,
                        "noise_reduction" if self.config.enable_noise_reduction else None,
    ...
    async def _resample_audio(self, audio_data: bytes, metadata: AudioMetadata) -> bytes:

...
1. **Call `mark_startup_complete()`** after initialization is complete
2. **Register dependencies** using `_health_manager.register_dependency()`
3. **Handle startup failures gracefully** without crashing the service
...
1. **Response Format**: Changed from simple fields to structured components
2. **Status Values**: Now supports "degraded" status
3. **Startup Behavior**: All services must call `mark_startup_complete()`
4. **Dependency Tracking**: Services must register dependencies
5. **Metrics**: New Prometheus metrics exposed

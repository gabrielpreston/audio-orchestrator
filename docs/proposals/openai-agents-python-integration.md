---
title: OpenAI Agents Python Integration
...
> Docs ▸ Proposals ▸ OpenAI Agents Python Integration
...
-  Isolate the library features that materially advance Discord Voice Lab’s roadmap—threaded
  ...
  【F:docs/roadmaps/2024-integrated-voice-devops.md†L14-L158】
-  Map those capabilities to current service responsibilities so we can target gaps without
  ...
  【F:docs/architecture/system-overview.md†L12-L41】【F:services/llm/app.py†L1-L217】
...
-  Marshal MCP tool calls for external systems while logging correlation metadata through the shared
  utilities package.
  【F:docs/architecture/system-overview.md†L26-L47】【F:services/llm/app.py†L55-L150】
-  Sustain fallback strategies so deployments without managed connectivity can continue using the
  ...
  【F:docs/roadmaps/2024-integrated-voice-devops.md†L88-L158】
...
-  **Threaded conversations**
  -  Benefit: Centralized history keeps Discord and MCP sessions consistent across turns.
    【F:docs/roadmaps/2024-integrated-voice-devops.md†L62-L158】
...
-  **Hosted dependency** — The managed runtime would introduce an external availability and data
   residency dependency that conflicts with the roadmap’s offline and fallback objectives.
   【F:docs/roadmaps/2024-integrated-voice-devops.md†L88-L158】
-  **Protocol impedance** — Discord integrations consume OpenAI-compatible responses; translating
   granular Agent events adds maintenance cost without delivering unique value over bespoke
   streaming enhancements.
   【F:services/llm/app.py†L85-L206】
-  **Observability gaps** — Success metrics in the roadmap (parity ledgers, transcript replays,
...
-  **Streaming envelope**: Extend the existing SSE or chunked-response logic in
  ...
  【F:services/llm/app.py†L85-L206】【F:docs/roadmaps/2024-integrated-voice-devops.md†L88-L158】
...
-  Regression ledgers and transcript replays show parity across backends, supporting the roadmap’s validation gates.【F:docs/roadmaps/2024-integrated-voice-devops.md†L88-L158】
-  Observability dashboards capture per-thread metrics, tool latencies, and retry counts without relying on hosted infrastructure.【F:docs/operations/observability.md†L9-L27】

## Open questions

-  Which datastore (Redis vs. PostgreSQL) best balances latency, durability, and existing operational tooling for conversation state?【F:docs/roadmaps/2024-integrated-voice-devops.md†L88-L158】
-  Do specific MCP tools require long-running orchestration that would benefit more from a task queue (Option C) than synchronous execution inside FastAPI?
...
-  What guardrails are necessary to keep optional hosted integrations (Agents or otherwise) from conflicting with the offline fallback mandated in the roadmap?【F:docs/roadmaps/2024-integrated-voice-devops.md†L88-L158】
